...
/// resolved for the entry date (null when no rate card covers it — a billable gap).
/// <paramref name="ModuleId"/>/<paramref name="ModuleName"/> are null for entries with no
/// module (non-modular projects, or the pre-module "unassigned" bucket);
/// <paramref name="ModuleDeleted"/> marks entries on a soft-deleted module.</summary>
...
public interface ITimeEntryRepository
{
    ...
    /// <summary>The single entry for a grid cell — one per (employee, project, module, date),
    /// where a null module is itself a bucket (non-modular projects / unassigned entries).</summary>
    Task<TimeEntry?> GetByCellAsync(Guid employeeId, Guid projectId, Guid? moduleId, DateOnly date, CancellationToken cancellationToken = default);
