    ...
    IBudgetAlertService budgetAlerts)
{
        ...
        Guid entryId, decimal? billedHours, CancellationToken cancellationToken = default)
    {
        ...
        // Entries on a fixed-price module bill the agreed amount, not hours × rate — no rate
        // needed. Everything else billable needs a rate: module override else project card.
        var onFixedPriceModule = entry.ModuleId is { } moduleId
            && (await modules.GetByIdAsync(moduleId, cancellationToken))?.IsFixedPrice == true;
        if (entry.IsBillable && !onFixedPriceModule)
        {
