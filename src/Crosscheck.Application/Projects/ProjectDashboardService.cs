    ...
    IReadOnlyList<RoleBudget> Roles)
{
    ...
    public decimal RemainingHours => AllocatedHours - SpentHours;
    public double PercentHours => AllocatedHours > 0 ? (double)(SpentHours / AllocatedHours) * 100 : 0;
    public bool IsOver => AllocatedHours > 0 && SpentHours > AllocatedHours;
}
...
public record BudgetStatus(
    ...
    decimal PendingHours,
    IReadOnlyList<RoleBudget> Roles,
    IReadOnlyList<ModuleBudget> Modules)
{
...
public class ProjectDashboard
{
    ...
    /// <summary>Per-module burn vs. allocation (empty for non-modular projects). Computed even
    /// when no budget row exists, so module burn always shows.</summary>
    public required IReadOnlyList<ModuleBudget> ByModule { get; init; }
    ...
    IModuleRepository modules)
{
    public async Task<ProjectDashboard?> GetAsync(Guid projectId, CancellationToken cancellationToken = default)
    {
        ...
        var budgetStatus = budget is null ? null : BudgetBurn.Compute(budget, liveModules, rows);
