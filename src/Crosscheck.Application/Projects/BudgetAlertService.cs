    ...
    ILogger<BudgetAlertService> logger) : IBudgetAlertService
{
    ...
    public async Task OnModuleChangedAsync(Guid projectId, Guid moduleId, CancellationToken cancellationToken = default)
    {
        try
        {
            var budget = await budgets.GetForProjectAsync(projectId, cancellationToken);
            if (budget is not null)
            {
                await alerts.ClearForBudgetAsync(budget.Id, $"module:{moduleId}:", cancellationToken);
            }
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Re-arming module alerts failed for project {ProjectId}, module {ModuleId}.", projectId, moduleId);
        }
    ...
    private async Task EvaluateCoreAsync(Guid projectId, CancellationToken cancellationToken)
    {
        ...
        if (budget is null)
        {
            return; // nothing to measure against — modules alone don't alert until a budget row exists
        }
        ...
        var status = BudgetBurn.Compute(budget, liveModules, rows);
        ...
        // Per-module hours burn (e.g. Supplemental Hours past 90% of its 82h). Keys always say
        // "module:" regardless of the project's display label.
        foreach (var module in status.Modules.Where(m => m.ModuleId is not null && !m.IsDeleted && m.AllocatedHours > 0))
        {
            foreach (var threshold in budget.AlertThresholds.OrderBy(t => t))
            {
                var key = $"module:{module.ModuleId}:pct:{threshold}";
                if (module.PercentHours >= threshold && !fired.Contains(key))
                {
                    pending.Add(new AlertIntent(key, threshold >= OpsThreshold, threshold, (decimal)module.PercentHours, Role: null, module));
                }
            }

            var moduleOverrunKey = $"module:{module.ModuleId}:overrun";
            if (module.IsOver && !fired.Contains(moduleOverrunKey))
            {
                pending.Add(new AlertIntent(moduleOverrunKey, NotifyOps: true, Threshold: null, (decimal)module.PercentHours, Role: null, module));
            }
        }
        ...
        foreach (var intent in pending)
        {
            ...
            {
                { Module: not null } => BuildModuleMessage(project, intent, recipients),
                { Role: not null } => BuildRoleMessage(project, intent, recipients),
                _ => BuildOverallMessage(project, status, intent, recipients),
            };
    ...
    /// (null for overrun), the burn percent, and the role or module it concerns (both null for
    /// project-overall).</summary>
    private sealed record AlertIntent(
        string Key, bool NotifyOps, int? Threshold, decimal BurnPercent,
        RoleBudget? Role, ModuleBudget? Module = null);
    ...
    private static EmailMessage BuildModuleMessage(
        Project project, AlertIntent intent, IReadOnlyList<string> recipients)
    {
        var module = intent.Module!;
        var label = Terminology.Singular(project.BreakdownLabel);
        var headline = intent.Key.EndsWith(":overrun")
            ? $"over its hours budget ({module.PercentHours.ToString("0", Usd)}%)"
            : $"at {intent.Threshold}% of its hours budget";

        var subject = $"[Budget] {project.Code} — {module.ModuleName} {label} {headline}";

        var remaining = module.RemainingHours;
        var lines = new List<string>
        {
            $"{project.Code} — {project.Name}: the {module.ModuleName} {label} is {headline}.",
            "",
            $"Hours: {module.SpentHours:0.##} of {module.AllocatedHours:0.##} spent "
                + (remaining >= 0 ? $"({remaining:0.##} remaining)" : $"({-remaining:0.##} over)"),
        };

        if (module.PendingHours > 0)
        {
            lines.Add($"Plus {module.PendingHours:0.##} hrs pending (open, not yet approved).");
        }

        if (module.IsFixedPrice)
        {
            lines.Add($"This {label} bills a fixed {module.Amount!.Value.ToString("C0", Usd)} — hours here are internal budget only.");
        }
