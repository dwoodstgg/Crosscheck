...
public class BudgetAlertRepository(NpgsqlDataSource dataSource) : IBudgetAlertRepository
{
    ...
    public async Task ClearForBudgetAsync(Guid budgetId, string keyPrefix, CancellationToken cancellationToken = default)
    {
        await using var connection = await dataSource.OpenConnectionAsync(cancellationToken);
        await connection.ExecuteAsync(new CommandDefinition(
            "DELETE FROM budget_alerts WHERE budget_id = @budgetId AND alert_key LIKE @pattern",
            new { budgetId, pattern = keyPrefix.Replace(@"\", @"\\").Replace("%", @"\%").Replace("_", @"\_") + "%" },
            cancellationToken: cancellationToken));
    }
}
