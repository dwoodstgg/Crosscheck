...
8. Projects ARE work orders, and a work order's budget breaks into **`project_modules`** (design decision #21) — called "modules" or "milestones" per project (`projects.breakdown_label`, display only; milestones show their sort number). Each module has an hour budget (flat `hours` OR Σ per-role `project_module_allocations`), optional rate overrides, and an optional agreed `amount`: set = fixed-price (bills exactly that; hours are internal budget; entries approve without a rate), null = T&M as incurred (how supplemental/out-of-scope hours work). Once a project has live modules, new entries MUST pick one (`time_entries.module_id`, one entry per employee+project+module+day, NULLS NOT DISTINCT); pre-module entries are a read-only "unassigned" bucket. Modules soft-delete only. Burn + threshold alerts run overall, per role, AND per module (`module:<id>:*` keys). The old standalone fixed-fee `milestones` table design is absorbed here (ready_to_bill workflow arrives with invoicing).
