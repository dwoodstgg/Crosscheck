...
## Excel timesheet import (built 2026-07-23)
Company workbook format (see design-doc.md §6.6; sample at `Samples/2026 Don Woods timesheet.xlsx`): `Yearly Info` sheet (employee, year, job list, holidays); two sheets per month — the calendar sheet JAN–DEC where hours are entered (project rows × day columns, leave rows, totals) and the paired `*-DESC` sheet that receives those hours rolled up and holds the typed per-project-per-day work descriptions. **The `-DESC` sheets are the import source** (label/date/hours/description in one place; revised 2026-07-23); the calendar is only cross-checked (per-project monthly totals, warning on mismatch — top block only). Hidden sheets and client-specific calendars are skipped; parser (`TimesheetWorkbookParser`, ClosedXML, cached formula values) skips #REF!/#VALUE! artifacts and zero rows and dedupes (project, date). Flow: upload (Ops/Admin, tenant email supplied — employee record created if missing, entra_oid linked on first sign-in) → staging rows (`timesheet_import_rows`) → **line-item review** (every row editable: project, date, hours, billing role, description, include; labels auto-map by project code then saved `timesheet_import_mappings`, hand-mapped labels remembered at commit; duplicates vs existing entries flagged and default-excluded; billing role defaults from assignment default else sole billable role) → commit (approved entries with import_id, module_id null = unassigned bucket, missing assignments auto-created, one transaction, bypasses window/assignment gates) → rollback until any entry invoiced; pending imports discardable. Workbook leave rows never appear on `-DESC` sheets (leave is derived on the timesheet, never stored as entries).
