...
8. Every project has a **`project_type`** (design decision #22) — `hourly` (billed for hours worked), `fixed_rate` (agreed price), or `service_contract` (set timeframe, billed monthly). The type drives what the budget form asks for: hourly = optional dollar/hours caps; fixed rate = the contract amount; service contract = a fixed `monthly_amount` whose contract total (monthly × months in start–end, `BudgetService.ContractMonths`) is stored in `budgets.amount` so burn/alerts measure the whole engagement and heavy/light months average out. The budget row's `type` mirrors the project type at save time. **"T&M" terminology is retired — never use it in UI, code, or docs; say "hourly".**
