...
2. Entries save as `open` and remain owner-editable. Periods are **semi-monthly** (1st–15th, 16th–EOM); Ops/Admin **closes the window** after each period ends (`timesheet_periods`), which locks employee edits for those dates. Closing/reopening a window is audited; reopening is how a straggler fixes a missed day after close.
3. Approver reviews and approves (bulk per project/period) → `approved`. Approval is the billing decision: `hours_billed` can be adjusted (worked 8, bill 6); `hours_worked` is never changed.
4. An approver can return an entry for correction with a comment (back to `open`, audited) instead of approving it.
